...
contract HeroFactory is SimpleFactory{
    ...
    function getHero() public view returns (Hero memory) {
        ...
        return heroes[heroLookup[msg.sender]];
    }
    ...
    function _createHero(string memory _name) internal {
        ...
        heroToOwner[id] = msg.sender; // Assign the hero we just created to the owner's address using the mapping.
        ownerHeroCount[msg.sender]++; // Increase the count which tracks how many heroes are owned by an address.
    ...
    function createHero(string memory _name) public {
        require((ownerHeroCount[msg.sender] == 0), "You already created a hero!"); // Can only have one hero!
        _createHero(_name);
    }
